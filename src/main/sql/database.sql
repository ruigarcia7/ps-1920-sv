...
if object_id('dbo.PracticeAthletes') is not null drop table dbo.PracticeAthletes
if object_id('dbo.TrainingAthletes') is not null drop table dbo.TrainingAthletes
if object_id('dbo.CalendarEntry') is not null drop table dbo.CalendarEntry
--Entidades Associativas
...
create table dbo.TrainingAthletes(
	Fk_Athlete_ID int foreign key references dbo.Athlete(Pk_Athlete_ID),
	Fk_TrainingSchedule_ID int foreign key references dbo.TrainingSchedule(Pk_TrainingSchedule_ID)
	primary key(Fk_Athlete_ID,Fk_TrainingSchedule_ID)
)


create table dbo.CalendarEntry(
	Pk_CalendarEntry_ID int identity(1,1) primary key,
	CalendarEntry_Date date,
	Fk_Game_ID int foreign key references dbo.Game(Pk_Game_ID),
	Fk_Event_ID int foreign key references dbo.Event(Pk_Event_ID),
	Fk_Tournament_ID int foreign key references dbo.Tournament(Pk_Tournament_ID)
)



/*ISSUES*/
/*[FIXED]Athlete-TrainingSchedule should be N-N.*/
/*[FIXED]Profile-Event should be N-N*/
